...
public class FootballLeague
{
    public static void Main()
    {
        ...
        var pattern = $@"{key}(?<firstTeam>.*?){key}.*{key}(?<secondTeam>.*?){key}.*?(?<firstTeamScore>\d+):(?<secondTeamScore>\d+)";
        ...
        while (!line.Equals("final"))
        {
            ...
            if (match.Success)
            {
                var firstTeam = new string(match.Groups["firstTeam"].Value
                    .ToUpper()
                    .Reverse()
                    .ToArray());

                var secondTeam = new string(match.Groups["secondTeam"].Value
                    .ToUpper()
                    .Reverse()
                    .ToArray());
